...
- Observability, a property of a system, is the ability to understand the internal state of a system by analyzing its external outputs e.g. logs, metrics, and traces.

- Telemetry: Data emitted by a system about itself is called Telemetry. Examples of telemetry are:
  - Logs
  - Metric
  - Trace

- Metrics: It involves tracking system metrics like CPU usage, memory usage, and network performance over time. It also provides alerts based on predefined thresholds and conditions.
  - Pros:
    - Metrics consume little storage.
    - They can be queried faster.
    - Great for alerting.
  - Cons:
    - No information about the root cause.
  > `Metrics tells us what is happening.`
- Logs: It involves the collection of detailed event records from various components of a system.
  - Pros:
    - Extremely detailed i.e. contains stack traces, exceptions, application behavior etc.
  - Cons:
    - Storage-heavy
    - Humans also struggle to manually read millions of log lines.
  > `Logging explains why it is happening.`

- Traces: It involves tracking the flow of a request or transaction as it moves through different services and components within a system. For example, if a user clicks **Checkout**, a trace records the entire request path:
  ```
  Frontend
    |
  API Gateway
  ...
  - Pros:
    ...
    Traces are perfect for:
    - microservices
    - latency analysis
    - dependency mapping
    - root cause analysis
  - Cons:
    - Expensive in terms of storage, network bandwidth, and processing costs.
  > `Tracing shows how it is happening.`

![Introduction to Observability](../assets/Introduction-to-Observability.png)

## 🤔 Why Monitoring?
- Monitoring helps us keep an eye on our systems to ensure they are working properly.
- Purpose:  Maintaining the **health, performance, and security** of IT environments.
- It enables early detection of issues, ensuring that they can be addressed before causing significant downtime or data loss.
- Monitoring relies on predefined criterions, we already know what we are looking for. E.g.
    - CPU usage > 90%
    - Disk space < 10%
...
## 🤔 Why Observability?
- Observability helps us understand why our systems are behaving the way they are. For example:
  - Users report slowness.
  - CPU looks normal.
  - Memory looks normal.
  - Database looks healthy.
  - Yet requests take 15 seconds.

- Observability helps investigate unknown problems. We don't know the question beforehand.
...
![why-monitoring-why-observability](../assets/why-monitoring-why-observability.png)
